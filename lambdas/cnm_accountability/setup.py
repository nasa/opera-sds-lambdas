...
class Package(setuptools.Command):
    ...
    ARCHIVE_NAME = "lambda-cnm_accountability-handler"
    ...
    def run(self):
        ...
        self.execute(
            "zip -9 {package_name} {files}".format(
                ...
                files=' '.join(
                    glob.glob("lambda_function.py") + glob.glob("queries.py") + glob.glob("*.jinja2")
                )))
...
setuptools.setup(
    name="lambda-cnm_accountability-handler",
    author="PCM",
    description="Lambda package for CNM delivery accountability and reporting",
